...
**Business question:** Predict whether a retail order is likely to be returned after delivery.

**Target:** `target` — `0` = Not returned, `1` = Returned (the order is predicted to be returned)
...
|---|---|---|---|---|---|
| `order_value` | 190.19 | 167.62 | 11.90 | 146.12 | 2659.39 |
| `item_count` | 2.98 | 1.71 | 0.00 | 3.00 | 9.00 |
| `discount_pct` | 50.43 | 19.37 | 0.00 | 49.56 | 100.00 |
| `customer_return_rate` | 50.75 | 19.73 | 0.00 | 51.37 | 100.00 |
| `delivery_days` | 29.71 | 17.44 | 0.00 | 30.00 | 60.00 |
| `category_risk` | 3.00 | 1.42 | 1.00 | 3.00 | 5.00 |

![Correlation heatmap](10_retail_return_risk/correlation_heatmap.png)
...
| Metric | Score |
|---|---|
| Accuracy | **67.00%** (baseline: 50.0%, lift: +17.0 pts) |
| Precision | 0.655 |
| Recall | 0.720 |
| F1-score | 0.686 |
| ROC-AUC | 0.731 |

![Confusion matrix](10_retail_return_risk/confusion_matrix.png)

**Confusion matrix breakdown** (test set, n=200):
- True Negatives: 62 (correctly predicted **Not returned**)
- True Positives: 72 (correctly predicted **Returned**)
- False Positives: 38 (predicted **Returned**, actually not returned — a "false alarm")
- False Negatives: 28 (predicted **Not returned**, actually returned — a "missed case")

![ROC curve](10_retail_return_risk/roc_curve.png)

An ROC-AUC of 0.731 means the model ranks a random positive case above a random negative case 73.1% of the time — well above the 50% (random-guess) line.
...
![Feature impact](10_retail_return_risk/coefficient_importance.png)

| Feature | Standardized coefficient | Odds ratio |
|---|---|---|
| `delivery_days` | +0.649 | 1.913 |
| `category_risk` | -0.612 | 0.542 |
| `discount_pct` | +0.535 | 1.707 |
| `order_value` | +0.500 | 1.649 |
| `item_count` | -0.435 | 0.647 |
| `customer_return_rate` | -0.349 | 0.706 |

**Top 3 drivers:**

- **`delivery_days`** — one standard deviation increase increases the odds of **returned** by roughly **91%** (odds ratio = 1.91).
- **`category_risk`** — one standard deviation increase decreases the odds of **returned** by roughly **46%** (odds ratio = 0.54).
- **`discount_pct`** — one standard deviation increase increases the odds of **returned** by roughly **71%** (odds ratio = 1.71).

*(A positive coefficient / odds ratio > 1 pushes toward `Returned`; negative / odds ratio < 1 pushes toward `Not returned`.)*
...
- **Threshold choice:** the default 0.5 probability threshold was used for classification. Depending on whether false positives (38) or false negatives (28) are more costly in this business context, the threshold could be tuned (e.g. via the ROC curve) to favor precision or recall.
...
With **67.0% test accuracy** and a **0.731 ROC-AUC** — a **+17.0 point** lift over the majority-class baseline — Logistic Regression provides a reasonably strong, fully interpretable model for **retail return risk**. Its main practical value here is not just prediction but **explainability**: the odds ratios above give a stakeholder a direct, defensible answer to "why did the model flag this case?" — something harder to get from a black-box model. For production use, the limitations above (especially threshold tuning and benchmarking against tree-based models) should be addressed first.
